...
export function deletePlan(workspace:Workspace,planId:string){
  ...
  return syncDerivedState({...workspace,plans,tasks,milestones,resources:workspace.resources.filter(item=>item.planId!==planId),notes:workspace.notes.filter(item=>item.planId!==planId),activity:workspace.activity.filter(item=>item.planId!==planId&&!item.taskId||!removedTaskIds.has(item.taskId??''))});
}
